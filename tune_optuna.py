...
def main() -> None:
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        width = trial.suggest_categorical("width", [96, 128, 160, 192])
        ...
        model = build_model(
            "cnn1d",
            dropout=dropout,
            hidden_channels=(width, width * 2, width * 2),
        ).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            train_metrics = train_one_epoch(model, train_loader, optimizer, device, "cnn1d")
            val_metrics = evaluate(model, val_loader, device, "cnn1d")
            history.append(
                {
                    "epoch": epoch,
                    "train": train_metrics,
    ...
    study = optuna.create_study(direction="maximize", study_name="protein_cnn1d_tuning")
    ...
    model = build_model(
        ...
        hidden_channels=(best_params["width"], best_params["width"] * 2, best_params["width"] * 2),
    ).to(device)
    ...
    for epoch in range(1, args.epochs + 3):
        train_metrics = train_one_epoch(model, train_loader, optimizer, device, "cnn1d")
        val_metrics = evaluate(model, val_loader, device, "cnn1d")
    ...
    test_metrics = evaluate(model, test_loader, device, "cnn1d")
