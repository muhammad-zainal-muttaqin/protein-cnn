# -*- coding: utf-8 -*-
"""Q3 protein secondary structure experiment — ResCNN + BiLSTM + Attention.
...
import os
# Ensure pip-installed cuDNN 9.3 (matching TF 2.19) is loaded before system cuDNN 9.1
...
run_config = {
    "base_dir": BASE_DIR, "output_root": OUTPUT_ROOT, "output_dir": OUTPUT_DIR,
    "train_file": TRAIN_FILE, "test_file": TEST_FILE, "seed": SEED,
    "max_len": MAX_LEN, "n_classes": N_CLASSES, "trials": TRIALS,
    "search_epochs": SEARCH_EPOCHS, "final_epochs": FINAL_EPOCHS,
    "val_fraction": VAL_FRACTION, "created_at": datetime.now().isoformat(timespec="seconds"),
}
...
assert cullpdb.shape[0] > cb513.shape[0], "Shapes look swapped — swap GDRIVE_ID_TRAIN / GDRIVE_ID_TEST and re-run."
...
class ValMacroF1Checkpoint(Callback):
    def __init__(self, x_val: np.ndarray, y_val_data: np.ndarray,
                 mask_val_data: np.ndarray, filepath: str | None = None,
...
def residual_conv_block(x: tf.Tensor, filters: int, kernel_size: int,
    ...
    x = SeparableConv1D(filters, kernel_size, padding="same",
...
def build_model(filters: int = 128, conv_blocks: int = 3, kernel_size: int = 7,
                lstm_units: int = 96, attention_heads: int = 4,
                attention_key_dim: int = 32, dense_units: int = 96,
                dropout: float = 0.30, weight_decay: float = 1e-5) -> Model:
    ...
    for block_idx in range(conv_blocks):
        x = residual_conv_block(x=x, filters=filters, kernel_size=kernel_size,
...
def compile_model(model: Model, lr: float, gamma: float,
...
def objective(trial: optuna.Trial) -> float:
    ...
    model.fit(X_train, y_train, sample_weight=sw_train,
              ...
              epochs=SEARCH_EPOCHS, batch_size=batch_size,
...
final_callbacks = [
    final_f1_callback,
    EarlyStopping(monitor="val_macro_f1", mode="max", patience=8,
                  restore_best_weights=False),
    ReduceLROnPlateau(monitor="val_macro_f1", mode="max", factor=0.5,
                      patience=3, min_lr=1e-6, verbose=1),
]

history = final_model.fit(X_train, y_train, sample_weight=sw_train,
                          validation_data=(X_val, y_val, sw_val),
...
with open(os.path.join(OUTPUT_DIR, "final_history.json"), "w", encoding="utf-8") as f:
    json.dump({key: [float(v) for v in values]
               for key, values in history.history.items()}, f, indent=2)
...
with open(os.path.join(OUTPUT_DIR, "classification_report.json"), "w", encoding="utf-8") as f:
    json.dump(classification_report(y_true_label, y_pred_label, digits=4,
                                    zero_division=0, output_dict=True), f, indent=2)
